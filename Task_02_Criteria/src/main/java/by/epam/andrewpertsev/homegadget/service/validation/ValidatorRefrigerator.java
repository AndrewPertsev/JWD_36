...
public class ValidatorRefrigerator implements Validable {

    @Override
    public boolean validateDevice(String groupName, String keyCriteriaMap, String valueCriteriaMap) {
        if (keyCriteriaMap.equals("OVERALL_CAPACITY")
                && (Double.parseDouble(valueCriteriaMap) <= 0
                | Double.parseDouble(valueCriteriaMap) > MAX_OVERALL_CAPACITY)) {
            PrintDeviceInfo.printMessage(groupName + " " + keyCriteriaMap + " can not exceed " + MAX_OVERALL_CAPACITY + ", or less than 0");
            return false;
        }
        ...
        if (keyCriteriaMap.equals("FREEZER_CAPACITY")
                && (Double.parseDouble(valueCriteriaMap) <= 0
                | Double.parseDouble(valueCriteriaMap) > MAX_FREEZER_CAPACITY)) {
            PrintDeviceInfo.printMessage(groupName + " " + keyCriteriaMap + " can not exceed " + MAX_FREEZER_CAPACITY + ", or less than 0");
            return false;
        }
